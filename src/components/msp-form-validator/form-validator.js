mspFrontend.FormValidator = function(form, options) {
  this.form = form;
  this.errors = [];
  this.validators = [];
...
mspFrontend.FormValidator.entityMap = {
  '&': '&amp;',
  '<': '&lt;',
  '>': '&gt;',
  '"': '&quot;',
...
mspFrontend.FormValidator.prototype.escapeHtml = function(string) {
  return String(string).replace(/[&<>"'`=\/]/g, function fromEntityMap (s) {
    return mspFrontend.FormValidator.entityMap[s];
  });
};

mspFrontend.FormValidator.prototype.resetTitle = function() {
  document.title = this.originalTitle;
};

mspFrontend.FormValidator.prototype.updateTitle = function() {
  document.title = "" + this.errors.length + " errors - " + document.title;
};

mspFrontend.FormValidator.prototype.showSummary = function () {
  this.summary.html(this.getSummaryHtml());
  this.summary.removeClass('msp-hidden');
  this.summary.attr('aria-labelledby', 'errorSummary-heading');
  this.summary.focus();
};

mspFrontend.FormValidator.prototype.getSummaryHtml = function() {
...
mspFrontend.FormValidator.prototype.hideSummary = function() {
  this.summary.addClass('msp-hidden');
  this.summary.removeAttr('aria-labelledby');
};

mspFrontend.FormValidator.prototype.onSubmit = function (e) {
...
mspFrontend.FormValidator.prototype.showInlineErrors = function() {
  for (var i = 0, j = this.errors.length; i < j; i++) {
    this.showInlineError(this.errors[i]);
  }
};

mspFrontend.FormValidator.prototype.showInlineError = function (error) {
  ...
  if(legend.length) {
    ...
    mspFrontend.addAttributeValue(fieldset[0], 'aria-describedby', errorSpanId);
  } else {
    label.after(errorSpan);
    control.attr('aria-invalid', 'true');
    mspFrontend.addAttributeValue(control[0], 'aria-describedby', errorSpanId);
  }
};

mspFrontend.FormValidator.prototype.removeInlineErrors = function() {
  var error;
  var i;
...
mspFrontend.FormValidator.prototype.removeInlineError = function(error) {
  var control = $("#" + error.fieldName);
  var fieldContainer = control.parents(".govuk-form-group");
  ...
  var errorSpanId = error.fieldName + '-error';
  mspFrontend.removeAttributeValue(fieldContainer.find('[aria-describedby]')[0], 'aria-describedby', errorSpanId);
};

mspFrontend.FormValidator.prototype.addValidator = function(fieldName, rules) {
...
mspFrontend.FormValidator.prototype.validate = function() {
  this.errors = [];
  var validator = null,
